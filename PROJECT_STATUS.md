...
## Just fixed (2026-08-25, twelfth pass) — the AI Overview staleness bug was NOT fully fixed by the tenth pass

**User confirmed the bug reproduced again after the tenth-pass fix**, with one critical extra detail that pinpointed it: the stale overview disappeared on a full page refresh but persisted through the in-app (SPA) navigation from AI Mode to a new query. That detail rules out a data/caching problem and points squarely at a client-side React state race — which is exactly what it was.

**The remaining gap**: the tenth-pass fix split AI Overview handling into a reset effect (`[query, type, page]`) and a populate effect (`[data, type]`). The populate effect's dependency array still included `type` — and `type` flips to `'web'` immediately on navigation, while `data` (from `useSearch`) keeps the *previous* type/query's response until the new fetch actually resolves. Since the prior AI-mode fetch for the same original query (e.g. "pasta") *also* has a valid `ai_overview.text_blocks` (same underlying Google response, just fetched under `type=ai`), the populate effect's condition (`type === 'web' && data?.ai_overview?.text_blocks`) evaluated true against that stale data the instant `type` changed — re-attaching the old overview in the *same render pass* as the reset, before it was ever visible. A hard refresh sidesteps this entirely because there's no stale `data` sitting around when everything starts fresh.

**Fix**: removed `type` from the populate effect's dependency array — it now only re-evaluates when `data` itself gets a genuinely new reference, which only happens once `useSearch`'s fetch for the *current* `[query, type, page]` has actually resolved. `type` is still read inside the effect body (just not listed as a dependency) — safe here specifically because `data` only ever changes as a *result* of a `[query, type, page]` change having already happened, so by the time this effect runs, the `type` in its closure is already current. `npm run build` clean.

**Not yet re-verified against the live deployed site** (no browser automation available in this session) — the previous "fixed" claim turned out to be incomplete, so treat this as fixed-pending-your-confirmation rather than closed. Please re-test the same repro (search → follow-up into AI Mode → new query in the search bar) once this is deployed.
