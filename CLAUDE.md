...
11. **`utils/markdown.js`'s placeholder tokens (`@@NEXACODEBLOCK...@@`, `@@NEXAINLINECODE...@@`) must never use a markdown syntax character** (`_`, `*`, `` ` ``, `~`, `[`, `]`). This already broke once — adding underscore-italic support corrupted the old `__CODE_BLOCK_N__` placeholder because the italic regex matched pieces of it. If you add a new markdown rule to this parser, check it doesn't match either placeholder format before shipping it. Same file/component also has `toPlainText()` helpers (in `NexaOverview.jsx` and inline in `markdown.js`'s callers) that exist specifically because SerpAPI/Gemini data can hand you an object where a string is expected — stringifying one directly produces the literal text `"[object Object]"` with no error, so it's easy to miss in testing.
