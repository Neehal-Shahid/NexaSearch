...
**Result-pack visual language (redesigned 2026-08-25, two passes)**: explicit product direction rejected the original bordered-card-grid treatment for video/image/news packs as too generic/Google-like. Current convention for these: **borderless tiles**, not boxes — `rounded-2xl`, a resting `shadow-[0_1px_2px_rgba(0,0,0,0.04)]`, and a much larger shadow plus a slight `-translate-y-1` lift on hover, instead of a border that changes color. Titles default to `text-text-primary` (calm) and only become `text-accent` on hover — never accent-colored at rest — matching the pattern `ShoppingResultGrid.jsx` already established before this work.

The first pass only restyled the existing grid (borders → shadows) and was correctly called out as still feeling too similar to the old layout. The pattern that actually reads as distinctive — confirmed working by user feedback on News, then deliberately extended to Video — is **editorial hierarchy, not uniform repetition**: one `featured` variant (large hero treatment — big thumbnail, title and snippet always visible, not hover-only) for the single lead item, followed by everything else in a secondary treatment (`list` for News: borderless row; the refined tile grid for Video). Both `NewsResultCard.jsx` and `VideoResultCard.jsx` follow this same `featured` + everything-else split now — treat this as the standard for any future result-pack redesign, not just a one-off for News. Images intentionally kept its existing masonry grid (no featured/list split) — it was never flagged as feeling generic the way Video/News were, and masonry itself already breaks uniformity by nature (variable tile heights).

**Video thumbnail resolution (fixed 2026-08-25)**: SerpAPI's own video thumbnails are genuinely tiny — verified by fetching and measuring one directly, ~148×83px — fine for a small grid cell, visibly blurry once stretched to fill a larger tile. Since most video results are YouTube links, `VideoResultCard.jsx` now extracts the video ID from `result.link` and requests YouTube's own `img.youtube.com/vi/{id}/sddefault.jpg` (640×480) instead, falling back to `hqdefault.jpg` then SerpAPI's original thumbnail on failure. Deliberately skips `maxresdefault` as the first attempt: YouTube returns a small gray placeholder at HTTP 200 (not an error) for videos without a true high-res thumbnail, which an `onError` fallback chain can't detect — `sddefault` doesn't have this failure mode. Also fixed in the same pass: the duration badge read `result.length`, but the real field is `result.duration` (verified against live data for both `inline_videos` and `video_results`) — the badge was likely never rendering at all before this.
