...
**Serverless function timeout (fixed 2026-08-25)**: `vercel.json` sets `functions["api/search.js"].maxDuration = 30` seconds. This exists because SerpAPI's `google_shopping` engine can be genuinely slow — verified live, a real "nike shoes" shopping search reported `search_metadata.total_time_taken: 34.29` seconds on SerpAPI's own side — and without an explicit `maxDuration`, the function ran on Vercel's platform default, well under that, causing the shopping tab to fail with "Search request failed" on slower queries. 30s is a conservative value chosen without verifying the account's actual plan tier (Hobby vs. Pro have different maximums) — if deploys fail on this config or shopping searches still occasionally time out, check the real limit in the Vercel dashboard.
