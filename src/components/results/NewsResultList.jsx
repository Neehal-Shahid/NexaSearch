...
export default function NewsResultList({ results, variant = 'default' }) {
  ...
  return (
    <div className={variant === 'inline' ? 'space-y-4' : 'space-y-10'}>
      {variant === 'default' && (
      ...
      <div className={variant === 'inline' ? 'flex flex-col gap-3' : 'grid grid-cols-1 md:grid-cols-2 gap-5'}>
        {visibleResults.map((result, index) => (
          <NewsResultCard key={result.link || index} result={result} variant={variant} />
        ))}
      </div>
